...
public class MiaoshaUserService {
    ...
    public boolean updatePassword(String token, long id, String formPass){
        ...
        toBeUpdate.setId(id);
        toBeUpdate.setPassword(MD5Util.formPassToDBFormPass(formPass,user.getSalt()));
        miaoshaUserDao.update(toBeUpdate);

        //处理缓存：上面只是修改了数据的密码，并没有删除redis中用户的缓存，并更新token
    ...
    //压测用：登录验证
    public String loginYace(HttpServletResponse httpServletResponse, LoginVo loginVo) {
        if(loginVo == null){
            throw new GlobalException(CodeMsg.SERVER_ERROR) ; //抛出 系统异常
        }
        String mobile = loginVo.getMobile();
